...
const AgentPlugin = require('./agent_plugin');
...
module.exports = class AgentRuntime extends IPCMessage {
  constructor(options, runtimer) {
    ...
    this.keepAliveTimer = setInterval(() => this.console.log(
      `[${this.pid}]`, 
      '[AGENT]',
      `'${this.options.name}' is keepalive`
    ), 24 * 60 * 60 * 1000);
  ...
  async onReceiveMessage(message) {
    ...
    await this.emit(message.action, ipc_service_data);
  }
  ...
  async listen() {
    ...
    if (fs.existsSync(this.options.plugin_file)) {
      ...
      const services = util.lang.installPlugin(PluginList, this.env, this.options.name, this.options.baseDir);
      for (let i = 0 ; i < services.length; i++) {
        const service = services[i];
        const component = new AgentPlugin(this, PluginConfigs[service.name]);
        this.serviceKeys.push(service.name);
        await service.exports(component, this);
        await component.poly();
        this.services[service.name] = component;
      }
    }
  ...
  async destroy(signal) {
    for (let i = 0; i < this.serviceKeys.length; i++) {
      const key = this.serviceKeys[i];
  ...
  kill(signal) {
    ...
    const timer = setInterval(() => {
      if (this.noticeToClose) {
        ...
        clearInterval(timer);
        this.console.info(`[${this.pid}]`, `[AGENT]`, this.options.name, `is stopping with signal '${signal}', please wait until process done ...`);
        this.destroy().then(() => {
