...
module.exports = class Master extends IPCMessage {
  ...
  async forkApplicationWorkProcess(max) {
    ...
    cluster.on('fork', worker => {
      ...
      this.console.info(`[${worker.process.pid}]`, '[WORKER]', 'forked');
    }).on('exit', worker => {
      this.console.info(`[${worker.process.pid}]`, '[WORKER]', 'exit');
  ...
  async forkAgentProcess() {
    ...
    for (let i = 0; i < this.options.agents.length; i++) {
      ...
      agent.on('exit', () => {
        this.fields.agents[name] = 0;
        this.console.info(`[${agent.pid}]`, '[AGENT]', `${name} exit`);
      });
      this.console.info(`[${agent.pid}]`, '[AGENT]', name, 'forked');
    }
  }
